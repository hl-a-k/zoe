...
/*
 * fork from :
 * https://github.com/extendedmind/logback-mongodb-appender
 */
package com.zoe.framework.logback.mongodb;
...
public class MongoDBAppender extends UnsynchronizedAppenderBase<ILoggingEvent> {
    ...
    private void connect() throws UnknownHostException {
        ...
        mongoDb = client.getDatabase(db == null ? "log" : db);
    }
    ...
    private Document getBasicLog(ILoggingEvent evt) {
        ...
        doc.put("timestamp", evt.getTimeStamp());
        doc.put("datetime", new Date(evt.getTimeStamp()));
        doc.put("level", evt.getLevel().toString());
        doc.put("thread", evt.getThreadName());
        doc.put("logger", evt.getLoggerName());
        Marker m = evt.getMarker();
        if (m != null) {
            doc.put("marker", m.getName());
        }
        doc.put("msg", evt.getFormattedMessage());
        if(evt.hasCallerData()) {
            StackTraceElement st = evt.getCallerData()[0];
            String callerData = String.format("%s.%s [%s : %d]", st.getClassName(), st.getMethodName(),st.getFileName(), st.getLineNumber());
            doc.put("caller", callerData);
        }
