...
public final class RegexInterpreter extends RegexRunner
{
	...
	public RegexInterpreter(RegexCode code, Locale culture)
	{
		runcode = code;
	...
	{
		char ch = (runrtl ? runtext.charAt(--runtextpos) : runtext.charAt(runtextpos++));

		return (runci ? Character.toLowerCase(ch) : ch);
	}
	...
	private boolean Stringmatch(String str)
	{
		...
		else
		{
			while (c != 0)
			{
				if (!(Character.toLowerCase(runtext.charAt(--pos)) == str.charAt(--c)))
				{
					return false;
				}
			}
	...
	private boolean Refmatch(int index, int len)
	{
		...
		else
		{
			while (c-- != 0)
			{
				if (!(Character.toLowerCase(runtext.charAt(--cmpos)) == Character.toLowerCase(runtext.charAt(--pos))))
				{
					return false;
				}
			}
	...
	protected boolean FindFirstChar()
	{
		...
		if (0 != (runanchors & (RegexFCD.Beginning | RegexFCD.Start | RegexFCD.EndZ | RegexFCD.End)))
		{
			...
			{
				if ((0 != (runanchors & RegexFCD.End) && runtextpos < runtextend) || (0 != (runanchors & RegexFCD.EndZ) && (runtextpos < runtextend - 1 || (runtextpos == runtextend - 1 && CharAt(runtextpos) != '\n'))) || (0 != (runanchors & RegexFCD.Start) && runtextpos < runtextstart))
				{
...
//C# TO JAVA CONVERTER TODO TASK: There is no 'goto' in Java:
				...
				case RegexCode.Ref:
				{
						...
						{
							if ((runregex.roptions.getValue() & RegexOptions.ECMAScript.getValue()) == 0)
							{
								break;
							}
						}
...
//C# TO JAVA CONVERTER TODO TASK: There is no 'goto' in Java:
				...
				default:
					throw new UnsupportedOperationException(SR.GetString(SR.UnimplementedState));
			}
