...
import com.zoe.framework.sql2o.data.LazyTable;
import com.zoe.framework.sql2o.data.Row;
import com.zoe.framework.sql2o.data.Table;
import com.zoe.framework.sql2o.data.TableResultSetIterator;
...
/**
 * Represents a sql2o preparedStatement. With sql2o, all statements are instances of the
 * Query class.
 */
...
public class Query implements AutoCloseable {
    ...
    public Query(Connection connection, String queryText, boolean returnGeneratedKeys, boolean parseQuery) {
        this(connection, queryText, returnGeneratedKeys, null, parseQuery);
    }

    private Query(Connection connection, String queryText,
                  boolean returnGeneratedKeys, String[] columnNames) {
        this(connection, queryText, false, columnNames, true);
    }

    private Query(Connection connection, String queryText,
                  boolean returnGeneratedKeys, String[] columnNames, boolean parseQuery) {
        ...
        paramNameToIdxMap = new LinkedHashMap<>();
        parameters = new LinkedHashMap<>();

        parsedQuery = parseQuery ? connection.getSql2o().getQuirks()
                .getSqlParameterParsingStrategy()
                .parseSql(queryText, paramNameToIdxMap) : queryText;
    ...
    private String getFormatedName() {
        if (name == null) {
            name = "Query_" + System.currentTimeMillis();
        }
    ...
    private void addParameterInternal(String name, ParameterSetter parameterSetter) {
        if (!this.getParamNameToIdxMap().containsKey(name)) {
            //not to throw new Sql2oException (@czc@20171106)
            logger.warn("Failed to add parameter with name '"
                    + name
    ...
    public Query addParameters(Map<String,Object> params) {
    ...
    public Query addParameter(String name, Object value) {
        return value == null ? addParameter(name, Object.class, null)
    ...
    /*public Query addParameter(String name, final List value) {
        try {
            if (value != null && value.size() > 0 && value.get(0) != null) {
                ServerType serverType = getConnection().getSql2o().getQuirks().getServerType();
                if (serverType.equals(ServerType.Oracle)) {
                    Class elClass = value.get(0).getClass();
                    String typeName = "VARCHAR";
                    if (String.class == elClass) {
                        typeName = "VARCHAR";
                    } else if (Integer.class == elClass || Long.class == elClass) {
                        typeName = "NUMBER";
                    }
                    final Array array = getConnection().getJdbcConnection().createArrayOf(typeName, value.toArray());
                    addParameterInternal(name, new ParameterSetter() {
                        @Override
                        public void setParameter(int paramIdx) throws SQLException {
                            getConnection().getSql2o().getQuirks()
                                    .setParameter(preparedStatement, paramIdx, array);
                        }
                    });
                }
            }
    ...
    public Query bind(final Object pojo) {
        ...
        Map<String, PojoProperty> propertyMap = PojoIntrospector
    ...
    /**
     * Iterable {@link java.sql.ResultSet} that wraps
     * {@link PojoResultSetIterator}.
     */
    private abstract class ResultSetIterableBase<T> implements
            ResultSetIterable<T> {
        private long start;
        ...
        public ResultSetIterableBase() {
            try {
                ...
                throw new Sql2oException("Database error: " + ex.getMessage(),
                        ex);
            }
        }
    ...
    /**
     * Read a collection lazily. Generally speaking, this should only be used if
     * you are reading MANY results and keeping them all in a Collection would
     * cause memory issues. You MUST call
     * {@link com.zoe.framework.sql2o.ResultSetIterable#close()} when you are
     * done iterating.
     *
    ...
    private <T> ResultSetHandlerFactory<T> newResultSetHandlerFactory(
            Class<T> returnType) {
    ...
    public <T> ResultSetIterable<T> executeAndFetchLazy(
        ...
        return new ResultSetIterableBase<T>() {
            public Iterator<T> iterator() {
                return new PojoResultSetIterator<>(rs, isCaseSensitive(),
                        quirks, resultSetHandlerFactory);
            }
        };
    }
    ...
    public <T> List<T> executeAndFetch(ResultSetHandlerFactory<T> factory) {
        ...
        // if sql2o moves to java 7 at some point, this could be much cleaner
    ...
    public <T> T executeAndFetchFirst(
            ResultSetHandlerFactory<T> resultSetHandlerFactory) {
        // if sql2o moves to java 7 at some point, this could be much cleaner
        // using try-with-resources
        ResultSetIterable<T> iterable = null;
        try {
        ...
        lt.setRows(new ResultSetIterableBase<Row>() {
            public Iterator<Row> iterator() {
                return new TableResultSetIterator(rs, isCaseSensitive(),
                        isAutoDeriveColumnNames(), getConnection().getSql2o()
                        .getQuirks(), lt);
            }
        });
        return lt;
    ...
    public Table executeAndFetchTable() {
        ...
        TableResultSetIterator iterator = (TableResultSetIterator) lt.rows().iterator();
        Table table = iterator.newTable();
        try {
    ...
    public Connection executeUpdate() {
        ...
        try {
            ...
            this.connection.setKeys(this.returnGeneratedKeys ? statement
                    .getGeneratedKeys() : null);
            connection.setCanGetKeys(this.returnGeneratedKeys);
        } catch (SQLException ex) {
            this.connection.onException();
            throw new Sql2oException("Error in executeUpdate, "
                    + ex.getMessage(), ex);
        } finally {
            closeConnectionIfNecessary();
        }
    ...
    @SuppressWarnings("unchecked")
    private <T> ResultSetHandler<T> newScalarResultSetHandler(
            final Class<T> returnType) {
        final Quirks quirks = getQuirks();
        try {
            final Converter<T> converter = throwIfNull(returnType,
            ...
            throw new Sql2oException("Can't get converter for type "
                    + returnType, e);
        }
    }
    ...
    public Query addToBatch() {
        try {
            ...
            throw new Sql2oException("Error while adding preparedStatement to batch", e);
        }
    ...
    public Connection executeBatch() throws Sql2oException {
        ...
        try {
            ...
            try {
                connection.setKeys(this.returnGeneratedKeys ? statement
                        .getGeneratedKeys() : null);
                connection.setCanGetKeys(this.returnGeneratedKeys);
            ...
            this.connection.onException();
            throw new Sql2oException("Error while executing batch operation: "
                    + e.getMessage(), e);
        } finally {
            closeConnectionIfNecessary();
        }
    ...
    public Query setColumnMappings(Map<String, String> mappings) {
        ...
        for (Map.Entry<String, String> entry : mappings.entrySet()) {
            this.caseSensitiveColumnMappings.put(entry.getKey(),
                    entry.getValue());
            this.columnMappings.put(entry.getKey().toLowerCase(), entry
    ...
    public Query addColumnMapping(String columnName, String propertyName) {
        ...
        this.columnMappings.put(columnName.toLowerCase(),
