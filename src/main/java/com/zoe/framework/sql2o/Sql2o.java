...
import com.zoe.framework.sql2o.quirks.QuirksDetector;
...
public class Sql2o {
    ...
    /**
     * Creates a new instance of the Sql2o class. Internally this constructor
     * will create a {@link GenericDatasource}, and call the
     * {@link Sql2o#Sql2o(javax.sql.DataSource)} constructor which takes a
    ...
    /**
     * Created a new instance of the Sql2o class. Internally this constructor
     * will create a {@link GenericDatasource}, and call the
     * {@link Sql2o#Sql2o(javax.sql.DataSource)} constructor which takes a
     * DataSource as parameter.
     *
     ...
     * @param user   database username
     * @param pass   database password
     * @param quirks {@link com.zoe.framework.sql2o.quirks.Quirks} allows sql2o to
     *               work around known quirks and issues in different JDBC drivers.
     */
    ...
    /**
     * Creates a new instance of the Sql2o class, which uses the given
     * DataSource to acquire connections to the database.
     *
    ...
    /**
     * Gets the default column mappings Map. column mappings added to this Map
     * are always available when Sql2o attempts to map between result sets and
     * object instances.
     *
     * @return The {@link Map<String, String>} instance, which Sql2o internally
     * uses to map column names with property names.
     */
     ...
     *
     * @param defaultColumnMappings A {@link Map} instance Sql2o uses internally to map between
     *                              column names and property names.
     */
    public void setDefaultColumnMappings(
            Map<String, String> defaultColumnMappings) {
     ...
     * mapping between columns names and property names. This should almost
     * always be false, because most relational databases are not case
     * sensitive.
     *
     ...
     * {@link com.zoe.framework.sql2o.Connection} class instead,
    ...
    public Query createQuery(String query, boolean returnGeneratedKeys) {
        return new Connection(this, true).createQuery(query,
                returnGeneratedKeys);
    }
     ...
     *
     * @return instance of the {@link com.zoe.framework.sql2o.Connection} class.
     */
     ...
     * {@link com.zoe.framework.sql2o.StatementRunnableWithResult} instance.
     * This method guarantees that the connection is closed properly, when
    ...
    public <V> V withConnection(StatementRunnableWithResult<V> runnable,
        ...
        try {
            ...
            throw new Sql2oException(
                    "An error occurred while executing StatementRunnable", t);
        } finally {
            if (connection != null) {
                connection.close();
            }
    ...
    /**
     * Begins a transaction with the given isolation level. Every statement
     * executed on the return {@link Connection} instance, will be executed in
     * the transaction. It is very important to always call either the
     * {@link com.zoe.framework.sql2o.Connection#commit()} method or the
     * {@link com.zoe.framework.sql2o.Connection#rollback()} method to close the
     * transaction. Use proper try-catch logic.
     *
     * @param isolationLevel the isolation level of the transaction
     * @return the {@link Connection} instance to use to run statements in the
     * transaction.
     */
    public Connection beginTransaction(int isolationLevel) {
        ...
        try {
            ...
            success = true;
            connection.setIsInSql2oTransaction(true);
     ...
     * {@link java.sql.Connection#TRANSACTION_READ_COMMITTED}. Every statement
     * executed on the return {@link Connection} instance, will be executed in
     * the transaction. It is very important to always call either the
     * {@link com.zoe.framework.sql2o.Connection#commit()} method or the
     * {@link com.zoe.framework.sql2o.Connection#rollback()} method to close the
    ...
    public Connection beginTransaction() {
        return this
                .beginTransaction(java.sql.Connection.TRANSACTION_READ_COMMITTED);
    }

    /**
     * Calls the {@link StatementRunnable#run(Connection, Object)} method on the
     * {@link StatementRunnable} parameter. All statements run on the
     * {@link Connection} instance in the
     * {@link StatementRunnable#run(Connection, Object) run} method will be
     * executed in a transaction. The transaction will automatically be
     * committed if the {@link StatementRunnable#run(Connection, Object) run}
     * method finishes without throwing an exception. If an exception is thrown
     * within the {@link StatementRunnable#run(Connection, Object) run} method,
     ...
     *                 {@link StatementRunnable#run(Connection, Object) run} method
     */
    public void runInTransaction(StatementRunnable runnable, Object argument) {
        runInTransaction(runnable, argument,
    ...
    public <V> V runInTransaction(
            StatementRunnableWithResult<V> runnableWithResult, Object argument) {
    ...
    public Date now() {
        try (Connection conn = this.open()) {
            String sql = this.getQuirks().getNowSql();
            return conn.createQuery(sql).executeScalar(Date.class);
        } catch (Exception ex) {
            return new Date();
        }
    }
     ...
     * @param readColumns 是否读取字段信息
     * @return
     */
    public List<Table> readMetaData(String schemaName, boolean readColumns) {
        try (Connection conn = this.open()) {
            return conn.readMetaData(schemaName, readColumns);
        }
    }
    ...
    public Table readMetaData(String schemaName, String tableName) {
        try (Connection conn = this.open()) {
            return conn.readMetaData(schemaName, tableName);
        }
    }

    /**
     * 设置在映射数据库字段到实体属性找不到对应属性时是否抛出错误
     *
     * @param throwOnMappingFailure 是否抛出错误
     */
    public void setThrowOnMappingFailure(boolean throwOnMappingFailure) {
        this.throwOnMappingFailure = throwOnMappingFailure;
    }

    /**
     * 获取在映射数据库字段到实体属性找不到对应属性时是否抛出错误
     *
     * @return
     */
