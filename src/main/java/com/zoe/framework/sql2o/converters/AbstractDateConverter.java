 ...
 */
public abstract class AbstractDateConverter<E extends Date> implements
		Converter<E> {
	...
	public E convert(Object val) throws ConverterException {
		...
		throw new ConverterException("Cannot convert type "
				+ val.getClass().toString() + " to java.util.Date");
	}

	public Timestamp toDatabaseParam(Date val) {
		if (val == null)
			return null;
		return (val instanceof Timestamp) ? (Timestamp) val : new Timestamp(
				val.getTime());
	}
}
