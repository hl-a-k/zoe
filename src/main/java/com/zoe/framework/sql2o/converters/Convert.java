...
import com.zoe.framework.sql2o.converters.joda.DateTimeConverter;
import com.zoe.framework.sql2o.converters.joda.LocalDateConverter;
import com.zoe.framework.sql2o.converters.joda.LocalTimeConverter;
import com.zoe.framework.sql2o.tools.FeatureDetector;

/**
 * Static class used to register new converters. Also used internally by sql2o
 * to lookup a converter.
 */
...
public class Convert {
	...
	private static void fillDefaults(Map<Class<?>, Converter<?>> mapToFill) {
		...
		mapToFill.put(java.sql.Timestamp.class,
				new AbstractDateConverter<java.sql.Timestamp>(
						java.sql.Timestamp.class) {
					@Override
					protected java.sql.Timestamp fromMilliseconds(
							long millisecond) {
	...
	static {
		...
		ServiceLoader<ConvertersProvider> loader = ServiceLoader
				.load(ConvertersProvider.class);
	...
	public static <E> Converter<E> throwIfNull(Class<E> clazz,
			Converter<E> converter) throws ConverterException {
		if (converter == null) {
			throw new ConverterException("No converter registered for class: "
					+ clazz.getName());
		}
		return converter;
	}
