package com.zoe.framework.sql2o.quirks.parameterparsing.impl;
...
public class ParameterParser implements CharParser {
    ...
    public boolean canParse(char c, String sql, int idx) {
        // 增加对数字型参数的支持（如“:0”)
        return idx + 1 < sql.length()
                && c == ':'
                && (Character.isJavaIdentifierStart(sql.charAt(idx + 1)) || Character
                .isDigit(sql.charAt(idx + 1)))
                && sql.charAt(idx - 1) != ':';
    }

    @Override
    public int parse(char c, int idx, StringBuilder parsedSql, String sql,
                     int length) {
        int startIdx = idx;
        idx += 1;
