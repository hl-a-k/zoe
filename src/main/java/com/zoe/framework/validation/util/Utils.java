package com.zoe.framework.validation.util;
...
public class Utils {
    ...
    public static Member getMember(Class<?> clazz, String propertyName){
        ...
        if(Map.class.isAssignableFrom(clazz)){
            return new MapMember(clazz, propertyName);
        }
        Member m ;
        try {
    ...
    static class MapMember implements Member {
        private final Class<?> clazz;
        private final String propertyName;

        public MapMember(Class<?> clazz, String propertyName) {
            this.clazz = clazz;
    ...
    public static Object getComparisonValue(PropertyValidatorContext context, Member memberToCompare, Object valueToCompare) {
        if(memberToCompare != null && context != null) {
            ...
            try {
                if(memberToCompare instanceof Field) {
                ...
                }else if(memberToCompare instanceof MapMember){
                    return ((MapMember)memberToCompare).get(targetInstance);
                }
            }catch (NoSuchFieldException | IllegalAccessException ex){
            }
        }
